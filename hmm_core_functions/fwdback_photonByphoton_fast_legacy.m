function [alpha, beta, gamma, loglik, xi_summed, gamma2] = fwdback_photonByphoton_fast_legacy(init_state_distrib, ...
    transmat, obslik, delta_t, Rho, transmat_t, varargin)
% FWDBACK Computes the posterior probs. in  H2MM using the forward backward algorithm.
%
...
%% Initializations
%  Xi parameter is calculated most of the time. As for gamma2, still check
%  what it means (Demian)
if 0 % nargout >= 5
...
%% Extract and initialize varaibles
...
for t=2:T
    ...
    % Extract tansition matrix product from pre-calculated list
    % "transmat_t"
    trans = transmat_t(:,:,delta_t(t));
    
    % Demian: check what maximize is
    if maximize
    ...
    % Demian: This is normally not used
...
% Note: If we use scaling, logliklihood is computed differently
if scaled
    if any(scale==0)
        loglik = -inf;
    else
...
%% Backward step and calculation of gamma and xi
...
for t = T-1:-1:1
    % b is temporary product
    b = beta(:,t+1) .* obslik(:,t+1);
    trans = transmat_t(:,:,delta_t(t+1));
    transs=trans;
    transs(transs==0)=1;
    ...
    % Calculate xi. Refer to Pirchi et al. (2016) paper
    
    if compute_xi
        % calculate first factor (xi star in paper)
        xi_temp = normalise((trans .* (alpha(:,t) * b')));
        
        % calculate second factor and multiply with xi_temp (Q: number of
        % states)
        
        fxi = reshape(sum(sum(repmat(xi_temp./transs,[1,1,Q,Q]).*Rho{delta_t(t+1)},1),2),[Q Q]);
        xi_summed = xi_summed + fxi;
    end
    %------ for gamma2 ------%
    if compute_gamma2
        denom = obslik(:,t) + (obslik(:,t)==0); % replace 0s with 1s before dividing
