% visualizeTPT_on_traces_cluster_Advanced.m
%
% This is the final visualization step in the CLUSTER analysis pipeline.
% It asks the user to select one of the four advanced TPT methods, calculates
% the boundaries, and launches an interactive viewer to display the binned
% trajectories with the TPTs shown as shaded regions.
%
function visualizeTPT_on_traces_cluster_Advanced()
    clearvars -except break_debug; close all; clc;
    fprintf('--- Advanced TPT Visualization Launcher (Cluster Workflow) ---\n');

    original_pwd = pwd;
    try
        %% 1. Select Processed Cluster Run Directory and Load All Data
        fprintf('\n--- 1. Selecting Processed Cluster HMM Run Directory ---\n');
        selected_run_path = uigetdir(pwd, 'Select the Cluster HMM Run directory');
        ...
        % --- Load ALL necessary cluster files ---
        fprintf('Loading all necessary result files for visualization...\n');
        load('Viterbi_decoded_states_Q0_cluster.mat', 'Q0');
        load('Viterbi_transition_analysis_cluster.mat', 'ind_trans', 'trans_num');
        load('data.mat', 'data');
        load('AA.mat', 'AA');
        load('HMM_parm.mat', 'K', 'Trans', 'Obs');
        load('Est.mat', 'B');
        ...
        P_Red_L = Obs(1); P_Red_H = Obs(2);
        state_map = zeros(1, max(B));
        ...
        for i_traj = 1:num_trajectories
            ...
            for k_trans = 1:length(transition_indices)
                ...
                switch params.method
                    ...
                    case 'Iterative LLR (Convergent)', [idx_exit, idx_entry] = findBoundaries_Iterative(@findExitPoint_Standard, @findEntryPoint_Standard, photon_colors, idx_change_viterbi, P_Red_H, P_Red_L, params.lambda_thresh, type, params);
                    ...
                    case 'Iterative Bayesian (Convergent)', [idx_exit, idx_entry] = findBoundaries_Iterative(@findExitPoint_Bayesian, @findEntryPoint_Bayesian, photon_colors, idx_change_viterbi, P_Red_H, P_Red_L, params.prob_thresh, type, params);
                end
        ...
        for i = 1:length(data), if ~isempty(data{i}), data_for_plotting{i}(:,1) = data{i}(:,1) * dt_model_sec; end; end
        
        AA_for_plotting = AA;

        Traj_Vet_local_with_TPT(data_for_plotting, Q0, ind_trans, trans_num, AA_for_plotting, dt_model_sec, Nstates, tpt_boundaries_per_traj);

        fprintf('\n--- Visualization Finished ---\n');
        cd(original_pwd);
    catch ME_TPT_Vis
        fprintf(2,'\nERROR: %s\n', ME_TPT_Vis.message); disp(ME_TPT_Vis.getReport());
        if exist('h_wait', 'var') && ishandle(h_wait), close(h_wait); end
        cd(original_pwd);
    end
end

%% --- TRAJECTORY VIEWER SUB-FUNCTION ---
function Traj_Vet_local_with_TPT(data_tv, Q0_tv, ind_trans_tv, trans_num_tv, AA_tv, dt_tv, Nstates_tv, tpt_boundaries_per_traj)
    ...
    fig_traj_vet = figure('Name', 'Trajectory Visualization with Viterbi & TPTs (Cluster)', 'Color', 'w', 'Position', [50, 50, 1200, 800]);
    ...
    for i_plot_loop = 1:num_traj_to_plot_actual
        ...
        X_bins_usec = 0 : Time_Res_us : Time_axis_photons_usec(end);
        ...
        linkaxes([ax1, ax2], 'x'); xlim(ax1, [0, Time_axis_photons_usec(end)]);
