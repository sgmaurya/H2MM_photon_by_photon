%% InitialGuess chain blink 3 colorsDouble
function [InitialGuess] = InitialGuess_chain_blink_3colorsDouble(number_of_initialguess,Nstates)
...
for state=Nstates
    ...
    for ii=1:number_of_initialguess
        ...
        template=zeros(state*2,state*2);
        F=rand(state,state);
        % optional:
        F=F-tril(F,-2)-triu(F,2); % making chain architecture

        template(1:state,1:state)=F;
        template(state+1:end,state+1:end)=F;

        % assign dark state transitions
        for k=1:state
            template(k,k+state)=rand;
            template(k+state,k)=rand;
            template(k+state,k+state)=rand;
        end
        ...
        expo = (10.^randi([3 6],state*2,1)); % random order of magnitude
