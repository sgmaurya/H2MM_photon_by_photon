% calculateTPT_Advanced_v4.m
%
% This is the final, fully functional version.
% It includes four advanced TPT calculation methods, a corrected user
% selection dialog, proper directory handling, and a complete, enhanced
% plotting and saving section.
%
function calculateTPT_Advanced_v2()
    clearvars -except break_debug; close all; clc;
    fprintf('--- Starting Advanced TPT Calculation (v4 - Final) ---\n');

    original_pwd = pwd; % Store the original directory
    try
        % --- Section 1: File Loading (with corrected path) ---
        fprintf('\n--- 1. Selecting HMM Run Directory and Loading Data ---\n');
        
        % --- FIX 1: Make the dialog start in the results folder ---
        project_base_path = fileparts(mfilename('fullpath'));
        results_base_path = fullfile(project_base_path, 'HMM_Local_Results');
        if ~exist(results_base_path, 'dir'), results_base_path = pwd; end % Fallback
        
        selected_run_path = uigetdir(results_base_path, 'Select the HMM_Run_* directory');
        if isequal(selected_run_path, 0), disp('User cancelled.'); return; end
        ...
        % --- Section 2: Parameter Prep (Unchanged) ---
        fprintf('\n--- 2. Preparing HMM Model Parameters ---\n');
        obsmat = model_params.obsmat; [~, sorted_idx] = sort(obsmat(:,2));
        ...
        fprintf('State Remapping: Original %d -> Low FRET, Original %d -> High FRET\n', sorted_idx(1), sorted_idx(2));

    % --- Section 3: User chooses method (Unchanged) ---
    ...
    switch analysis_method
        case 'Standard LLR'
            ans_lambda = inputdlg({'LLR Threshold:'}, 'Input', [1 50], {'3.4'});
        ...
        case 'Iterative LLR (Convergent)'
            ans_iter = inputdlg({'LLR Threshold:', 'Max Iterations:', 'Tolerance (photons):'}, 'Input', [1 60], {'3.4', '10', '2'});
            ...
            title_param_str = sprintf('Iterative LLR, Lambda=%.2f', params.lambda_thresh);
        case 'Standard Bayesian'
            ans_prob = inputdlg({'Probability Threshold:'}, 'Input', [1 50], {'0.99'});
        ...
        case 'Iterative Bayesian (Convergent)'
            ans_iter = inputdlg({'Probability Threshold:', 'Max Iterations:', 'Tolerance (photons):'}, 'Input', [1 60], {'0.99', '10', '2'});
            if isempty(ans_iter), cd(original_pwd); return; end
            ...
            title_param_str = sprintf('Iterative Bayes, P_{thresh}=%.2f', params.prob_thresh);
    end

    % --- Section 4: Main Calculation Loop (Unchanged) ---
    fprintf('\n--- 4. Calculating TPTs using ''%s'' ---\n', params.method);
    % ... (The main for-loop with the switch-case for the 4 methods is identical to the last version) ...
    ...
    for i_traj = 1:num_trajectories
        ...
        for k_trans = 1:length(transition_indices)
            ...
            switch params.method
                case 'Standard LLR', idx_exit = findExitPoint_Standard(photon_colors, idx_change_viterbi, P_Red_H, P_Red_L, params.lambda_thresh, type); idx_entry = findEntryPoint_Standard(photon_colors, idx_change_viterbi, P_Red_H, P_Red_L, params.lambda_thresh, type);
                case 'Iterative LLR (Convergent)', current_center_idx = idx_change_viterbi; for iter = 1:params.max_iter, exit_pass = findExitPoint_Standard(photon_colors, current_center_idx, P_Red_H, P_Red_L, params.lambda_thresh, type); entry_pass = findEntryPoint_Standard(photon_colors, current_center_idx, P_Red_H, P_Red_L, params.lambda_thresh, type); if isnan(exit_pass) || isnan(entry_pass), break; end; new_center_idx = round((exit_pass + entry_pass) / 2); if abs(new_center_idx - current_center_idx) <= params.conv_tol_photons, idx_exit = exit_pass; idx_entry = entry_pass; break; end; current_center_idx = new_center_idx; end; if isnan(idx_exit), idx_exit = exit_pass; idx_entry = entry_pass; end
                case 'Standard Bayesian', idx_exit = findExitPoint_Bayesian(photon_colors, idx_change_viterbi, P_Red_H, P_Red_L, params.prob_thresh, type); idx_entry = findEntryPoint_Bayesian(photon_colors, idx_change_viterbi, P_Red_H, P_Red_L, params.prob_thresh, type);
                case 'Iterative Bayesian (Convergent)', current_center_idx = idx_change_viterbi; for iter = 1:params.max_iter, exit_pass = findExitPoint_Bayesian(photon_colors, current_center_idx, P_Red_H, P_Red_L, params.prob_thresh, type); entry_pass = findEntryPoint_Bayesian(photon_colors, current_center_idx, P_Red_H, P_Red_L, params.prob_thresh, type); if isnan(exit_pass) || isnan(entry_pass), break; end; new_center_idx = round((exit_pass + entry_pass) / 2); if abs(new_center_idx - current_center_idx) <= params.conv_tol_photons, idx_exit = exit_pass; idx_entry = entry_pass; break; end; current_center_idx = new_center_idx; end; if isnan(idx_exit), idx_exit = exit_pass; idx_entry = entry_pass; end
            end
    ...
    % --- FIX 2: Re-integrated the full, enhanced plotting and saving section ---
    ...
    bin_width_ans = inputdlg('Enter bin width in microseconds (e.g., 2):', 'Bin Width', [1 40], {'2'});
    ...
    title(ax, {title_str1, title_str2}, 'Interpreter', 'none'); % Use 'none' to be safe with underscores etc.
    ...
    fprintf('Results saved to .mat and .png files with method name.\n');
        ...
        fprintf(2,'\nERROR in calculateTPT_Advanced_v4: %s\n', ME_TPT_Adv.message);
        disp(ME_TPT_Adv.getReport());
        if exist('h_wait', 'var') && ishandle(h_wait), close(h_wait); end
    end
    
    cd(original_pwd); % Ensure we always return to the original directory
end
